...
let concater = function(formula) {
	...
	if(!formula.protons) {
		for(let i in formula) {
			concater(formulareal, formula[i].material);
			formulareal = formulareal.concat(formula[i].count);
		}
	} else if(!formula.neutrons) {
		formulareal = formulareal.concat(formula.formula);
	} else if(formula.neutrons) {
		formulareal = formulareal.concat(formula.formula);
	}
	return formulareal;
